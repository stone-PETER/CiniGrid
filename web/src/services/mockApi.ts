...
const mockSuggestions: Suggestion[] = [
  {
    ...
    description:
      "A sleek 20-story glass building in the heart of downtown with panoramic city views, modern amenities, and excellent public transport access.",
  ...
  },
  {
    ...
    description:
      "Industrial converted warehouse space with exposed brick walls, high ceilings, and large windows. Perfect for creative projects.",
  ...
  },
  {
    ...
    description:
      "Stunning rooftop location with city skyline views, upscale furnishings, and professional lighting setup available.",
...
let mockPotentialLocations: Location[] = [
  {
    ...
    description:
      "Elegant pre-war apartment with park views and classic NYC charm.",
    coordinates: { lat: 40.7829, lng: -73.9654 },
    region: "Upper West Side",
    tags: ["apartment", "pre-war", "park-view"],
    permits: [{ name: "Filming Permit", required: true }],
    images: [
  ...
  },
  {
    ...
    description:
      "Charming waterfront cafe with iconic bridge views and cozy interior.",
...
let mockFinalizedLocations: Location[] = [
  {
    ...
    description:
      "Professional studio space in the heart of Times Square with controlled lighting.",
...
let mockNotes: Note[] = [
  {
    _id: "note-1",
    text: "Great natural lighting in the afternoon. Would work well for dialogue scenes.",
    author: {
      _id: "user-scout-1",
      username: "scout1",
      role: "scout",
  ...
  {
    _id: "note-2",
    text: "Parking might be an issue during rush hours. Consider weekend shoots.",
    author: {
      _id: "user-manager-1",
      username: "manager1",
      role: "manager",
  ...
  {
    _id: "note-3",
    text: "Owner is very cooperative and familiar with film productions.",
    author: {
      _id: "user-scout-1",
      username: "scout1",
      role: "scout",
...
export const mockApiService = {
  ...
  auth: {
    login: async (credentials: LoginRequest): Promise<ApiResponse<User>> => {
      ...
      const user = mockUsers.find(
        (u) =>
          u.username === credentials.username && u.role === credentials.role
      );
  ...
  locations: {
    ...
    addPotentialLocation: async (
      ...
      const newLocation: Location = {
        ...
        permits:
          location.manualData?.permits?.map((p) => ({
            name: p,
            required: true,
          })) || [],
    ...
    finalizeLocation: async (id: string): Promise<ApiResponse<Location>> => {
      ...
      const locationIndex = mockPotentialLocations.findIndex(
        (l) => l._id === id
      );
      if (locationIndex === -1) {
        throw new Error("Location not found");
  ...
  approvals: {
    getApprovals: async (
      ...
      await delay(200);
      const approvals = mockApprovals.filter(
    ...
    addApproval: async (
      ...
      const existingIndex = mockApprovals.findIndex(
        (a) =>
          a.locationId === approvalData.locationId &&
          a.role === newApproval.role
      );
